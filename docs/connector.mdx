...
- **Personal access token** (syncs info from one or more Databricks workspaces)
...
- Account ID
- One or more personal access tokens
- Deployment name of each Databricks workspace you're syncing (the subdomain in the workspace URL, not the workspace ID), listed in the same order as the corresponding tokens
